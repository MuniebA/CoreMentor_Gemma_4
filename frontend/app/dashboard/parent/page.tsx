                ...
                {selectedChild && (
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {children.map(child => (
                                <Link
                                    href={`/dashboard/parent/child/${child.student_id}`}
                                    key={child.student_id}
                                    className="bg-white rounded-xl shadow-sm p-5 hover:shadow-md border border-gray-100 transition"
                                >
                                    <p className="font-semibold text-gray-800">{child.full_name}</p>
                                    <p className="text-sm text-gray-500 mt-2">Level {child.level} &middot; {child.rank}</p>
                                    <p className="text-xs text-blue-600 mt-4 font-medium">Open full record &rarr;</p>
                                </Link>
                            ))}
                        </div>
